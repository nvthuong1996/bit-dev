...
EOD`;
  ...
  publishComponent(componentName: string, componentVersion = '0.0.1') {
    ...
    const componentFullName = componentName;
    ...
    logger.error('====================', extractedDir);
    this.helper.command.runCmd('npm publish', extractedDir);
  }

  unpublishComponent(packageName: string) {
    this.helper.command.runCmd(`npm unpublish @bit/${this.helper.scopes.remote}.${packageName} --force`);
  }
